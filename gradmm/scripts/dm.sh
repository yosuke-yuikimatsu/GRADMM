...
dm_num_projectors=1
dm_by_class=true
# Example random-BERT configuration:
...
full_tag=${dataset}-${split}-${MODEL}-nreal${subset_size}-steps${n_steps}-nsyn${n_gen}-${opt_alg}-rho${admm_rho}-inner${admm_inner_steps}-dm_${dm_mode}_${dm_projector}_w${dm_weight}-seed${rng_seed}
...
CUDA_VISIBLE_DEVICES=$gpu python generate.py \
    ...
    --dm_match $dm_match \
    --dm_num_projectors $dm_num_projectors \
    --dm_by_class $dm_by_class 2>&1 | tee -a ${LOG_DIR}/${full_tag}_${current_time}.log
